...
const HEADING_PATTERNS = {
  ...
  // X.Y / X.Y.Z 节（支持顿号、点、空格、"摇"分隔）
  section: /^\s*(\d+(?:\.\d+)+)\s*[、．.\s摇]+(.+)$/,
  // 单独数字章节：1 引言 / 1. 引言（严格过滤带页码的目录项、正文片段）
  // 更严格验证：数字后面不应是纯页码引用（如"1 5"），也不应是正文片段（以"所示""网络中""而采用"等开头）
  // 支持"摇"作为分隔符：部分中文PDF字体将空格映射为"摇"字符（U+6447），导致标题格式为"1摇 深度学习"
  numericChapter: /^\s*(\d{1,2})\s*[\.．、\s摇]\s*(?!(?:\d{1,4}\s*$|所示|网络中|而采用|如图|如表|通过|根据|由于|因此|所以|因为|如果|虽然|但是|然而|使得|导致|本文|本书|本章|本节|可以看出|可以得出|可见|由此|据此|综上|总之))(.{2,80})$/,
  // 中文数字 + 顿号 + 短标题
...
// 正文片段开头词：以这些词开头的行更可能是正文句子而非标题
const BODY_FRAGMENT_PREFIXES = /^(?:所示|网络中|而采用|如图|如表|通过|根据|由于|因此|所以|因为|如果|虽然|但是|然而|使得|导致|本文|本书|本章|本节|可以看出|可以得出|可以认为|可见|由此|据此|综上|总之|此外|另外|同时|并且|而且|不仅|鉴于)/;

// 学术论文引用/参考文献特征标记：包含"[数字]"引用、"in Chinese"、"SIMARD"、"STEINKRAUS"等
const CITATION_PATTERN = /\[\d+\]|\bin\s+Chinese\b|SIMARD|STEINKRAUS|KORF|BURLING|GREFENSTETTE|et\s+al\.|ibid\.|op\.?\s*cit\.|参考文献|References|BIBLIOGRAPHY/i;

// PDF 乱码字符特征：数字后紧跟异常汉字（如"底""架"等），或特定字出现在不恰当位置
// 注意："摇"字符在部分中文PDF中替代空格作为标题分隔符（如"1摇 深度学习"），不应视为乱码
const PDF_GARBAGE_PATTERN = /^\s*底[^\d]|[\d．.]+\s*[底架拓扑韧辙褪]/;

// PDF 乱码文本特征2：单个汉字被空格分隔（如"出 底 种 底 循 环"），或方括号后跟非数字内容
const PDF_FRAGMENT_PATTERN = /[\u4e00-\u9fa5]\s+[\u4e00-\u9fa5]\s+[\u4e00-\u9fa5]\s+[\u4e00-\u9fa5]|[[【][^\d\]】]|[A-Z]\.\s*[A-Z][a-z]+\s+(?:等|和|与)/;
...
function isGarbageHeading(line) {
  ...
  // 11. 过长的正文片段：标题超过80个字符且包含句号/逗号等正文标点，应视为正文而非标题
  if (trimmed.length > 80 && /[。，；！？、,;!?]/.test(trimmed)) return true;

  // 12. 参考文献/引用：包含"[数字]"引用标记、"in Chinese"、"SIMARD"、"STEINKRAUS"等学术论文引用特征
  if (CITATION_PATTERN.test(trimmed)) return true;

  // 13. PDF乱码字符：包含非常用字符（如"摇"出现在数字后面、"底"出现在不恰当位置）
  if (PDF_GARBAGE_PATTERN.test(trimmed)) return true;

  // 14. 正文句子片段：以"所示"、"网络中"、"而采用"等正文词汇开头的
  if (BODY_FRAGMENT_PREFIXES.test(stripFontMarkers(trimmed))) return true;

  // 15. PDF碎片化文本：单个汉字被空格分隔，或方括号后跟非数字内容，或学术引用格式
  if (PDF_FRAGMENT_PATTERN.test(trimmed)) return true;

  // 16. 尾部碎片标题：通常是更长标题的断裂尾部
  //     如 "势进行展望"（"6 总结与展望" 的尾部）、"估指标的定量结果"（截断片段）
  //     特征：不含编号前缀，且以非起始字开头（这些字几乎不会作为完整标题首字）
  const coreFrag = stripFontMarkers(trimmed);
  if (coreFrag.length >= 5 && coreFrag.length <= 20 &&
      !/^\s*(?:\d+\.?|[一二三四五六七八九十]+[、．.]|第\s*[一二三四五六七八九十\d]+\s*[章节])/.test(coreFrag)) {
    // "势" 几乎不会作为标题首字（总是出现在"趋势""态势""优势"等的第二位）
    if (/^势/.test(coreFrag)) return true;
    // "估" 后不接"算""计"时，说明是"评估"断裂后的碎片
    if (/^估(?!算|计)/.test(coreFrag)) return true;
  }

  // 17. 正文片段特征：标题包含正文连接短语，说明是正文句子而非标题
  //     如 "方法的融合,具有一定的研究意义" 中的 "具有一定的"
  //     如 "与其他方法的融合等" 中的 "与其他" 和 "等" 结尾
  if (/具有一定的|等人|具有.{0,10}意义|与其他|融合等$/.test(stripFontMarkers(trimmed))) return true;

  // 18. 逗号后接长文本的正文片段（规则11仅处理>80字，此处补充较短的情况）
  //     如 "方法的融合,具有一定的研究意义"（逗号后跟5字以上中文）
  if (/[,，]\s*[\u4e00-\u9fa5]{5,}/.test(stripFontMarkers(trimmed)) && trimmed.length <= 60) return true;

  // 19. 截断标题：以"的+单字"结尾但不构成完整词
  //     如 "其他方法的融"（应为"其他方法的融合"）
  const coreTrunc = stripFontMarkers(trimmed);
  if (coreTrunc.length <= 15 && /的融$/.test(coreTrunc) && !/的融合$/.test(coreTrunc)) return true;
...
function isHeadingLike(line, neighbors = null) {
  ...
  if (matchedType && NEEDS_CONTEXT_VERIFICATION.has(matchedType)) {
    if (!hasHeadingContextEvidence(trimmed, neighbors, matchedType)) {
      matchedType = null;
    }
  }
 ...
 */
function hasHeadingContextEvidence(trimmed, neighbors, matchedType) {
  // 去掉字号标记后判断
  ...
  // 2.5. 强编号模式放宽：numericChapter 匹配且编号为 1-20、标题含中文且长度合理时，
  //      即使上下文行短（双栏 PDF 布局导致行交错）也直接通过
  //      这是标题最可靠的特征之一，不应因布局问题被过滤
  if (matchedType === 'numericChapter') {
    const m = core.match(HEADING_PATTERNS.numericChapter);
    if (m) {
      const num = parseInt(m[1], 10);
      const titleText = (m[2] || '').replace(/摇/g, '').trim();
      if (num >= 1 && num <= 20 && titleText.length >= 2 && titleText.length <= 40 && /[\u4e00-\u9fa5]/.test(titleText)) {
        return true;
      }
    }
  }
...
function parseHeading(line, type, options = {}) {
  ...
  if (type === 'numericChapter') {
    const m = trimmed.match(HEADING_PATTERNS.numericChapter);
    const title = m ? cleanTitle(m[2].replace(/摇/g, '')) : cleanTitle(trimmed);
    return { level: 1, title, number: m ? m[1] : '', raw: trimmed };
  }
